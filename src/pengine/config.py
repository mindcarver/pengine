...
class Settings(BaseSettings):
    ...
    # Hard-pin OpenRouter chat-completions routing to a single upstream provider
    # slug (request-level provider.order with allow_fallbacks=false). Unpinned
    # routing gambles every large cold-prefill call across ~15 upstreams, and the
    # slow ones queue for minutes until the router's idle ceiling kills the
    # stream before any content chunk (Issue #285). Empty keeps OpenRouter's
    # default price-weighted load balancing.
