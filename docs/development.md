...
- **多集任务拆成单集请求**（2026-09-23，run d52cb948 ep4）。整组 sidecar 提取调用让
  温度 0 上游"只写第一集就停手"（ep31/ep4 两次生产实录，nonce 扰动与 provider 轮转
  均无效——"停在第 1 集"这个行为本身扛得住一切输入扰动）。解法：`_invoke_script_group_sidecar`
