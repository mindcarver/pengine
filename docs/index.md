...
| 接入本地服务 | [HTTP API](api/) | 8 个公开端点、请求示例、幂等键、状态和稳定错误 |
...
- 剧情合同通过确定性校验和绑定的独立审核；
- 每一集都绑定合同、前序剧本和折叠后的 `SeriesState`，并通过确定性与语义连续性检查；
- 完整剧本批次通过绑定的全剧结构审核；
...
| 人格 manifest 与九文件 hash | [`contracts/persona-package.schema.json`](https://github.com/mindcarver/pengine/blob/main/contracts/persona-package.schema.json) 与 `src/pengine/personas.py` |
