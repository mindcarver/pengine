...
当前 `src/pengine/repository.py` 的迁移头版本为 `31`。下表按领域归纳表，而不是要求调用方直接依赖内部 SQL；SQL schema 变化必须通过迁移维护。
...
| 设计版本 | `series_bible_candidates`、`series_bible_lineage`、`series_bible_projection_repairs` | SeriesBible 候选、active/stale/superseded、重建预算；投影修复机器已按 #222 退役，表仅保留历史证据 |
