...
`## 第N集` Markdown 正文（无害的标题装饰——集号后带副标题、标记两侧空格、首标题前
导语——会在解析前被确定性归一化，归一化文本才是持久化与哈希对象），再针对固定正文提取事实、时间线、线索和分集义务
Sidecar；三次解析全部失败时，模型原文会作为不可变证据落库以便排障。逐组独立审查是守门员而非审计师：仅拦硬 Canon／锁定上游矛盾、组边界越界、义务链断裂、线索生命周期不可能与创作者确认的 L4 硬规则；组内时间线/知识微张力、派生年龄算术、节奏类检查与无锁定来源依据的观感问题永不构成拒绝理由。每组输入由大纲上下文编译器白名单组装：保留用户请求、对应
...
不会重新生成；现有 LangGraph `thread_id` 会继续用于恢复。整体墙钟时限与阶段调用预算都按工作单元重置（大纲每开始一个自然组、剧本每开始一集；剧本阶段的全剧调用总量也按自然组重置），因此默认 30 分钟时限 / 48 次单集上限 / 192 次单组总量对任意集数同样适用。首次整体墙钟超时或限定的
