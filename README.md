...
SHA-256 才会写入业务检查点并锁定。后续每集读取该合同、active SeriesBible、上一集
折叠后的 `series_state` 和已锁剧本；统一 SeriesBible 流程先执行逐集确定性合同/状态
校验，再原子提交剧本、`episode_state_delta`、新状态及其哈希。`series_reviewer` 只在
声明的结构里程碑和最终完整前缀运行；`episode_reviewer` 保留给没有 active SeriesBible
的兼容路径，不是统一流程每集必调的模型。Skill 只加载到对应审查或修复子代理，不作为
监督 Agent 的全局提示。
...
次数、模型身份确认与内容修复次数分别记录，互不消耗。响应模型身份不是配置模型、
缺失或出现多个值时，本次响应会被丢弃并暂停为 `relay_identity_mismatch`；只能在核验
Relay 后人工继续。只有全部分集锁定、聚合哈希复验通过，
...
> 调用预算为生成 `192`、审核 `128`；统一流程的审核调用量取决于合同、里程碑、终审和
> 修复次数。预算只是出站上限，不能证明 80 集规划或交付已经可靠。
